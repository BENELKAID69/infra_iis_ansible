<#
.SYNOPSIS
    Nom du script : 03.InstallWinRM-FQDN-CA-Optimedit.ps1 (Version V6.4-MultiAdmin-Smart)
    Rôle : Centraliser, orchestrer et pousser la configuration WinRM HTTPS sur le parc (Cibles mixtes : DCs & Serveurs).
...
.DESCRIPTION
    ...
    5. Détection dynamique du rôle système (Contrôleur de Domaine vs Serveur Membre).
    6. Élévation du GROUPE AD au rang d'Administrateur (SAM locale ou Builtin AD selon le rôle).
    7. Déclenchement à distance de l'exécution du script local esclave via Invoke-Command.
    8. Consolidation finale des logs historiques de configuration et validation des écouteurs.
#>
...
# Déclaration de votre parc mixte (Contrôleurs de domaine et serveurs membres)
$Servers       = @("OPT-DC02.optimedit.eu")
...
Write-Host "`n--- Configuration des serveurs cibles (Application dynamique des privilèges) ---" -ForegroundColor Cyan
...
foreach ($Server in $Servers) {
    ...
    if (Test-Connection -ComputerName $Server -Count 1 -Quiet) {
        ...
        # Exécution à distance de la logique de configuration adaptative
        Invoke-Command -ComputerName $Server -ScriptBlock {
            param($ADGroupName, $DomainName, $LeafName)
            
            # 1. Détermination du rôle de la machine hôte
            ...
            if ($IsDC) {
                Write-Host "  [DETECTION] L'hôte cible est un Contrôleur de Domaine (DC)." -ForegroundColor Yellow
                try {
                    # Importation du module ActiveDirectory nécessaire sur le DC
                    Import-Module ActiveDirectory -ErrorAction SilentlyContinue
                    
                    # Ajout dans le groupe Administrateurs BUILTIN de l'AD (Spécifique aux DCs)
                    if (-not (Get-ADGroupMember -Identity "Administrateurs" | Where-Object { $_.SamAccountName -eq $ADGroupName })) {
                        Add-ADGroupMember -Identity "Administrateurs" -Members $ADGroupName -ErrorAction Stop
                        Write-Host "  [OK] Le groupe AD '$ADGroupName' a été injecté dans Builtin\Administrateurs de l'AD." -ForegroundColor Green
                    } else {
                        Write-Host "  [INFO] Le groupe AD '$ADGroupName' est déjà membre de Builtin\Administrateurs." -ForegroundColor Yellow
                    }
                } catch {
                    Write-Host "  [ERREUR] Échec de l'assignation des privilèges AD : $($_.Exception.Message)" -ForegroundColor Red
                }
            } else {
                Write-Host "  [DETECTION] L'hôte cible est un Serveur Membre standard." -ForegroundColor S過a
                try {
                    # AJOUT DU GROUPE AD dans la base SAM locale du serveur membre
            ...
            # 2. Déclenchement de la configuration locale esclave V5.2
            $RemoteScript = "C:\temp\$LeafName"
            if (Test-Path $RemoteScript) {
                Write-Host "  [EXEC] Lancement de la configuration WinRM sur l'hôte distant..." -ForegroundColor Cyan
                & $RemoteScript
            }
